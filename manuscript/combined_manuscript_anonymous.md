...
<!-- LEGACY ANONYMOUS ASSEMBLED EXPORT. Canonical source: manuscript/comprehensive_manuscript.md; see manuscript/README.md. -->
...
All analyses used R v4.5.2 (R Core Team 2024) with key packages vegan 2.7.2, MASS 7.3.65, piecewiseSEM 2.3.1, DHARMa 0.4.7, boot 1.3.32, sandwich 3.1.1, betapart 1.6.1, and mediation 4.5.1 (full session information archived with data). Significance was assessed at α = 0.05 throughout. Natural logarithms were used for all log transformations; site was included as a fixed effect because three levels provide too few for reliable random-effect estimation. Additional details on multiple-testing correction, diagnostics, bootstrap implementation, and secondary analyses are provided in the Supplementary Methods.

### Q1: Scaling

CAFI abundance was modelled as a power law (N = aV^β^) using negative binomial GLMs with log link (total CAFI ~ log(volume) + site); the log(volume) coefficient directly estimates β. We contrasted proportional, sublinear, and super-linear scaling. Species richness was modelled analogously using a Poisson GLM, yielding the species--area relationship exponent *z*. Rarefied richness (n = 20) was regressed on log(volume) via ordinary least-squares. Species- and group-level scaling analyses are reported in the supplement.

### Q2: Composition

Compositional differences were tested using marginal PERMANOVA on Bray--Curtis distances from Hellinger-transformed abundances, with PERMDISP for dispersion homogeneity. Continuous size effects were tested with db-RDA and variance partitioning; nestedness was tested with NODF against quasiswap null matrices and beta diversity was decomposed into turnover and nestedness-resultant components. Additional robustness checks are reported in the Supplementary Methods.

### Q3: Diversity and condition

We tested whether CAFI diversity predicts coral condition using OLS models (condition PC1 ~ predictor + log(volume) + site). Count predictors were square-root transformed; richness was untransformed. We used joint richness-and-abundance models, variance partitioning, and piecewiseSEM path analysis to separate diversity from abundance effects. Rarefied richness was compared to raw richness as an abundance control. Detailed diagnostics, mediation, host-identity sensitivity analyses, species-level tests, reverse-direction models, and cross-study concordance analyses are described in the Supplementary Methods.

### Neighbourhood context

For the 61 neighbourhood-surveyed colonies, we modeled abundance, richness, and Shannon diversity as functions of log(volume), neighbour count, total neighbour volume, mean neighbour distance, and site. The distance--richness relationship was retested with rarefied richness. Additional neighbourhood, co-occurrence, host-identity, community-assembly, and robustness analyses are described in the Supplementary Methods.
...
**Fig. 1.** Study design and representative CAFI. (A) Mo'orea reef sites: Hauru (n = 38), Maatea (n = 39), Maharepa (n = 35). (B) Colony volume distribution (log10 scale; 21--42,333 cm³; n = 112). (C) Neighbourhood density (conspecifics within 5 m; n = 61). (D--F) Representative CAFI: *Trapezia* sp. guard crab, *Harpiliopsis spinigera* shrimp, *Neocirrhites armatus* hawkfish.
