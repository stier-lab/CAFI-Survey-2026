...
Species richness positively predicted coral condition (p = 0.018), but the association is modest (adjusted R² = 0.04) and its mechanistic interpretation remains uncertain. The signal is entangled with abundance (shared explanatory power 70.8%). Rarefied richness showed no relationship (Fig. S8), but this null cannot adjudicate between the two interpretations: if diversity enhances condition partly through total occupancy -- more species means more defenders, more nutrient subsidizers, and fewer empty niches -- then rarefaction strips the mechanism along with the confound. The reduced sample (n = 47; minimum detectable R² = 0.15 at 80% power) further limits inferential leverage. The large residual variance in composition (91.5% unexplained by site and volume) means that much of the community variation driving any diversity-condition link arises from stochastic colonization history rather than deterministic size-dependent assembly.
