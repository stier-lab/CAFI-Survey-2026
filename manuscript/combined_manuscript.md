...
**Q2 (Composition):** Does CAFI composition change along the coral size gradient through species turnover or nestedness, and do communities converge or diverge as corals grow? We predicted turnover rather than nestedness, with colony volume and host architecture structuring occupant identity beyond abundance alone. If assembly is deterministic, communities on larger corals should converge toward predictable compositions -- a prerequisite for any diversity--condition feedback.
...
To test whether community variability changes with coral size, we computed multivariate dispersion (distance to group centroid in Bray--Curtis space; vegan::betadisper) within volume terciles (Small, Medium, Large). Decreasing dispersion with size indicates compositional convergence (deterministic filtering); increasing dispersion indicates divergence (priority effects or historical contingency). We tested for differences using ANOVA on distances to centroid and assessed monotonic trends using Spearman rank correlation. We extended the variance partitioning to three predictor sets: [E] coral size (log volume), [A] host architecture (branch width and genetic species identity from mtORF haplotype), and [S] space (site), using vegan::varpart on Bray--Curtis dissimilarities. Each fraction's significance was tested by db-RDA with 999 permutations.
...
#### Host species identity and CAFI associations (Supplement)

We tested whether coral genetic species identity predicts CAFI community structure beyond colony size and site effects. Analyses used the 98 successfully genotyped colonies after excluding *P. acuta* (n = 1) and ungenotyped colonies (n = 15).

We tested species effects on CAFI composition using marginal PERMANOVA (Bray--Curtis, 999 permutations) with poc_species, log(volume), and site as predictors. Pairwise PERMANOVA between all six species pairs used FDR-corrected p-values (Table S16). We partitioned community variation among three predictor sets -- coral species identity, log(volume), and site -- using vegan::varpart on Hellinger-transformed abundances and tested each fraction by db-RDA (999 permutations). A second partition replaced species with branch width to assess whether architecture mediates genotype effects on composition (Table S17).

To identify individual CAFI taxa associated with host species, we screened 52 OTUs present in ≥5 corals using negative binomial GLMs (OTU abundance ~ poc_species + log(volume) + site) with FDR correction across all tests. For OTUs showing significant species effects, we tested whether adding branch width eliminated the species term, classifying responses as architecture-mediated or genuine genotype effects (Table S18; Fig. S12). We identified indicator OTUs for each coral species using indicspecies::multipatt (IndVal.g index; 999 permutations) and computed a host specificity index as Simpson's diversity of host usage against a null expectation from 999 randomizations shuffling CAFI across corals (Table S19; Fig. S13).

We tested whether coral branch architecture mediates host species effects on *Trapezia* body size using a linear mixed model (carapace width ~ branch_width + poc_species + (1|coral_id)), controlling for crab species identity. We assessed the *Trapezia* species × branch architecture association using a chi-square contingency test and tested character displacement of *T. serenei* body size (alone vs. with co-occurring congeners) using a Wilcoxon rank-sum test (Fig. S14).

#### Community assembly (Supplement)

We quantified the balance between deterministic and stochastic assembly using the Raup--Crick metric (vegan::raupcrick; 999 iterations; Chase et al. 2011). Raup--Crick values near 0.5 indicate stochastic assembly; values significantly below 0.5 indicate communities more similar than expected (environmental filtering); values above 0.5 indicate greater-than-expected dissimilarity. We tested whether co-occurring CAFI are more or less taxonomically related than expected using standardized effect sizes of mean pairwise distance (SES.MPD, equivalent to the Net Relatedness Index) and mean nearest taxon distance (SES.MNTD, equivalent to the Nearest Taxon Index; picante::ses.mpd, ses.mntd; 999 randomizations using the 'taxa.labels' null model). Taxonomic distances were computed from the hierarchical classification (type > family > genus > species) using UPGMA clustering (ape::as.phylo). We tested for distance-decay of community similarity using Mantel tests (Bray--Curtis dissimilarity vs. geographic distance; Spearman correlation; 9,999 permutations) on the 57 colonies with GPS coordinates, with a partial Mantel controlling for coral size.
...
Yet site and volume together explain only ~14% of composition. Three-way variance partitioning reveals that host architecture -- branch width and genetic species identity -- explains more unique compositional variation (5.6%) than colony size (4.6%) or site (2.9%) alone (Table S16; Supplementary Methods), indicating that habitat identity, not merely habitat area, structures these communities. Mutualist and antagonist CAFI responded differently to host identity: the corallivore *Galeropsis monodonta* showed strong host specificity to *P. verrucosa* (7-fold enrichment), while obligate mutualists colonized based on geometry rather than host species (see Q3 discussion below). Volume-weighted null models detected no significant pairwise associations after FDR correction (Fig. S9), but six species showed significant mating-pair aggregation, concentrating conspecific pairs on a subset of corals. This non-random mosaic of well-defended and poorly-defended colonies -- arising from pair-aggregation within a stochastic backdrop -- creates the among-colony variation in functional composition on which any diversity-condition relationship depends.

The decline in beta-dispersion from small to large corals (ANOVA F = 9.21, p < 0.001; Fig. S15) adds a temporal dimension to the assembly picture. If colony size proxies community age — larger colonies having had longer for post-settlement sorting — then communities progress from stochastic early colonization toward deterministic convergence, a trajectory predicted by metacommunity theory for habitat patches above a threshold size [@Chase2011]. This convergence makes the diversity–condition relationship (Q3) mechanistically coherent: complementarity requires repeatable species combinations, not random assemblages, and the predictable communities that emerge on large corals provide exactly the stable functional backdrop on which diversity effects can operate.

### CAFI diversity and coral condition

Species richness positively predicted coral condition (p = 0.018), but the association is modest (adjusted R² = 0.04) and its mechanistic interpretation remains uncertain. The signal is entangled with abundance (shared explanatory power 70.8%). Rarefied richness showed no relationship (Fig. S8), but this null cannot adjudicate between the two interpretations: if diversity enhances condition partly through total occupancy -- more species means more defenders, more nutrient subsidizers, and fewer empty niches -- then rarefaction strips the mechanism along with the confound. The reduced sample (n = 47, 55% power) further limits inferential leverage. Although Raup-Crick null models indicate that CAFI assembly is broadly deterministic (see above), 85% of compositional variation remains unexplained by size, architecture, and space, meaning that much of the community variation driving any diversity--condition link reflects processes not captured by our predictors.
...
See separate document: `combined_supplement.md` (14 supplementary figures, 15 supplementary tables, supplementary methods).
