...
save_figure(p_lisa, file.path(FIG_DIR, "lisa_clusters.png"), width = 10, height = 8)
...
# Determine which sites are excluded (no spatial coordinate data)
excluded_sites <- setdiff(c("HAU", "MAT", "MRB"), unique(spatial_data$site))
exclusion_note <- if (length(excluded_sites) > 0) {
  paste0("| ", paste(excluded_sites, collapse = ", "), " excluded: no spatial coordinate data available")
} else {
  ""
}
...
if (length(common_corals) > 20) {
  ...
  tryCatch({
    ...
    save_figure(p_decay, file.path(FIG_DIR, "distance_decay.png"), width = 8, height = 6)
    ...
    save_figure(p_placeholder, file.path(FIG_DIR, "distance_decay.png"), width = 8, height = 6)
