#!/usr/bin/env Rscript
# ============================================================================
# 05_coral_condition.R - Coral Physiological Condition Analysis
# ============================================================================
#
# PURPOSE: Develop position-corrected coral condition score and analyze
#   how coral condition varies with landscape characteristics
#
# SAMPLING POSITION BIAS:
#   Sampling position (stump_length) correlates with colony size (r=0.565).
#   All physiological metrics are corrected for this confound before analysis.
#
# ANALYSES:
#   1. Position correction via residualization
#   2. PCA to create composite condition score (PC1)
#   3. Condition vs landscape (size, proximity, site)
#
...
#   - Panel A: Condition vs coral size
#   - Panel B: Condition vs isolation
...
cat("\n========================================\n")
cat("05: Coral Condition Analysis\n")
cat("========================================\n\n")
...
fig_dir <- file.path(FIGURES_DIR, "condition")
...
physio_full <- physio_clean %>%
  left_join(coral_master %>%
              dplyr::select(coral_id, volume_lab, depth_m, site_coral = site, branch_width,
...
outliers <- physio_full %>%
  dplyr::select(coral_id, site, all_of(physio_vars)) %>%
...
write_csv(qc_summary %>% pivot_longer(everything(), names_to = "metric", values_to = "value"),
          file.path(TABLES_DIR, "physiology_qc_summary.csv"))
if(nrow(outliers) > 0) {
  write_csv(outliers, file.path(TABLES_DIR, "physiology_outliers.csv"))
}
...
if(nrow(position_bias_data) > 10) {
  ...
  write_csv(position_by_size,
            file.path(TABLES_DIR, "sampling_position_by_size_quintile.csv"))
  ...
  p_bias <- ggplot(position_bias_data, aes(x = volume_lab, y = stump_length)) +
    ...
    geom_smooth(method = "lm", color = "#D55E00", se = TRUE, linewidth = 1) +
    scale_x_log10(labels = scales::comma) +
    labs(
      title = "Sampling position bias",
    ...
    theme_publication() +
    theme(
      plot.title = element_text(color = "#D55E00"),
...
correction_data <- physio_full %>%
  ...
  dplyr::select(coral_id, site, volume_lab, stump_length, all_of(physio_vars)) %>%
...
corrected_traits <- correction_data %>%
  dplyr::select(coral_id, site, volume_lab, stump_length)
...
cor_matrix <- cor(corrected_traits %>% dplyr::select(all_of(corrected_vars)))
...
write_csv(as.data.frame(cor_matrix) %>% rownames_to_column("trait"),
          file.path(TABLES_DIR, "physiology_correlations_corrected.csv"))
...
pca_corrected <- prcomp(corrected_traits %>% dplyr::select(all_of(corrected_vars)),
...
p_scree <- ggplot(scree_data, aes(x = PC, y = Variance)) +
  ...
  geom_line(aes(group = 1), linewidth = 1, color = "darkblue") +
  geom_point(size = 3, color = "darkblue") +
  geom_text(aes(label = sprintf("%.1f%%", Variance)),
            vjust = -0.5, fontface = "bold", size = 4) +
  scale_y_continuous(limits = c(0, max(variance_explained) * 1.15), expand = c(0, 0)) +
  labs(
...
pca_scores <- as.data.frame(pca_corrected$x[, 1:2]) %>%
  bind_cols(corrected_traits %>% dplyr::select(coral_id, site))
...
# Save PCA results
write_csv(scree_data, file.path(TABLES_DIR, "pca_variance_explained_corrected.csv"))
write_csv(loadings, file.path(TABLES_DIR, "pca_loadings_corrected.csv"))
write_csv(pca_scores, file.path(TABLES_DIR, "pca_scores_corrected.csv"))
...
write_csv(all_size_models,
          file.path(TABLES_DIR, "size_effects_on_corrected_traits.csv"))
...
p_size_effects <- ggplot(corrected_long, aes(x = volume_lab, y = value)) +
  ...
  geom_smooth(method = "lm", color = "#009E73", se = TRUE, linewidth = 0.8) +
...
write_csv(loadings_df, file.path(TABLES_DIR, "condition_score_loadings.csv"))
...
p_condition_site <- ggplot(corrected_traits, aes(x = site, y = condition_score, fill = site)) +
  ...
  scale_fill_site() +
  labs(
    title = "Coral condition score by site",
...
if(sum(!is.na(corrected_traits$volume_lab)) > 20) {
  ...
  write_csv(condition_summary,
            file.path(TABLES_DIR, "condition_score_size_model.csv"))
}
...
trait_profiles <- corrected_traits_categorized %>%
  dplyr::select(condition_category, all_of(corrected_vars)) %>%
  pivot_longer(cols = all_of(corrected_vars),
               names_to = "trait",
               values_to = "value") %>%
  mutate(
...
p_trait_profiles <- ggplot(trait_profiles,
  ...
  scale_fill_gradient2(low = "#D55E00", mid = "white", high = "#009E73",
...
write_csv(condition_summary_by_site,
          file.path(TABLES_DIR, "condition_score_summary_by_site.csv"))

# Save complete dataset
corrected_traits_export <- corrected_traits %>%
  dplyr::select(coral_id, site, volume_lab, stump_length, condition_score,
                all_of(corrected_vars))

write_csv(corrected_traits_export,
          file.path(TABLES_DIR, "coral_condition_scores.csv"))

saveRDS(corrected_traits_export,
        file.path(OBJECTS_DIR, "coral_condition_scores.rds"))
...
# 1. Sampling position bias correlation
if(exists("cor_test")) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "Bias",
      question = "Is sampling position confounded with colony size?",
      test_name = "Pearson correlation (volume vs stump_length)",
      test_statistic = cor_test$statistic,
      df = cor_test$parameter,
      p_value = cor_test$p.value,
      effect_size = cor_vol_stump,
      effect_type = "r",
      n = nrow(position_bias_data),
      notes = "Significant bias: larger colonies sampled higher on branch"
    )
  )
}
...
stats_results <- bind_rows(stats_results,
  create_result_row(
    hypothesis = "PC1",
    question = "How much variance does PC1 capture?",
    test_name = "PCA variance explained",
    test_statistic = variance_explained[1],
    df = "4 traits",
    p_value = NA,
    effect_size = variance_explained[1] / 100,
    effect_type = "Proportion",
    n = nrow(corrected_traits),
    notes = sprintf("PC1=%.1f%%, PC2=%.1f%%", variance_explained[1], variance_explained[2])
  )
)

# 3. Size effect on condition score (if model exists)
if(exists("condition_size_model")) {
  cond_summary <- summary(condition_size_model)
  coef_row <- coef(cond_summary)["log10(volume_lab)", ]

  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H_size",
      question = "Does colony size affect condition score after position correction?",
      test_name = "Linear regression (condition ~ log10(volume))",
      test_statistic = coef_row["t value"],
      df = condition_size_model$df.residual,
      p_value = coef_row["Pr(>|t|)"],
      effect_size = cond_summary$r.squared,
      effect_type = "R²",
      n = nrow(corrected_traits %>% filter(!is.na(volume_lab))),
      notes = sprintf("β=%.3f (should be ~0 if correction worked)", coef_row["Estimate"])
    )
  )
}

# 4. Site effect on condition (ANOVA)
if(nrow(corrected_traits) > 10 && sum(!is.na(corrected_traits$site)) > 0) {
  condition_site_aov <- aov(condition_score ~ site,
                            data = corrected_traits %>% filter(!is.na(site)))
  aov_summary <- summary(condition_site_aov)[[1]]

  # Calculate eta-squared
  ss_site <- aov_summary["site", "Sum Sq"]
  ...
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H_site",
      question = "Does coral condition differ among sites?",
      test_name = "One-way ANOVA",
      test_statistic = aov_summary["site", "F value"],
      df = paste(aov_summary["site", "Df"], aov_summary["Residuals", "Df"], sep = ", "),
      p_value = aov_summary["site", "Pr(>F)"],
      effect_size = eta_sq,
      effect_type = "η²",
      n = sum(!is.na(corrected_traits$site)),
      notes = "Tests for among-site differences in condition"
    )
  )
}

# 5. Size effect models for individual traits
if(exists("all_size_models") && nrow(all_size_models) > 0) {
  for(i in 1:nrow(all_size_models)) {
    row <- all_size_models[i, ]
    stats_results <- bind_rows(stats_results,
      create_result_row(
        hypothesis = paste0("H_size_", row$trait),
        question = paste("Does colony size affect", row$trait, "after position correction?"),
        test_name = "Linear regression",
        test_statistic = row$estimate / row$std.error,  # t-value
        df = NA,
        p_value = row$p.value,
        effect_size = row$r_squared,
        effect_type = "R²",
        n = NA,
        notes = sprintf("β=%.3f", row$estimate)
      )
    )
  }
}
...
save_stats_summary(stats_results, "05_coral_condition", "Coral Condition Analysis")
