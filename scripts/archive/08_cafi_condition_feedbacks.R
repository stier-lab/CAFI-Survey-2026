#!/usr/bin/env Rscript
# ============================================================================
# 08_cafi_condition_feedbacks.R - CAFI -> Coral Condition Feedbacks
# ============================================================================
#
# PURPOSE: Test whether CAFI communities affect coral physiological condition
#
# CORE QUESTION: Do CAFI benefit coral health?
#
# HYPOTHESES:
#   - Defenders (Trapezia) -> higher condition (predator defense, cleaning)
#   - Resident fish -> higher condition (nutrient subsidies)
#   - Corallivores -> lower condition (tissue consumption)
#
# ANALYSES:
#   - Total CAFI abundance vs condition
#   - Functional group effects (defenders, fish, corallivores)
#   - Community composition (NMDS1) vs condition
#   - Multiple regression with all groups
#
# MANUSCRIPT FIGURES:
#   >>> FIGURE 6: CAFI -> Coral Condition Feedbacks <<<
#   - Panel A: Defender abundance vs condition
#   - Panel B: Corallivore abundance vs condition
#   - Panel C: Functional group effect sizes
#
# DEPENDENCIES: 00_setup.R, 01_load_clean_data.R, 05_coral_condition.R
#
...
cat("\n========================================\n")
cat("08: CAFI-Condition Feedbacks\n")
cat("========================================\n\n")
...
fig_dir <- file.path(FIGURES_DIR, "feedbacks")
...
# Merge all datasets
# Note: volume_lab is coral volume from coral_master
analysis_data <- condition_scores %>%
  ...
  left_join(coral_master %>% select(coral_id, coral_volume = volume,
...
# Save results
write_csv(univariate_df, file.path(TABLES_DIR, "cafi_predicts_condition_univariate.csv"))
...
capture.output(
  ...
  file = file.path(TABLES_DIR, "cafi_predicts_condition_multiple_regression.txt")
)
...
write_csv(stats_summary, file.path(TABLES_DIR, "cafi_predicts_condition_stats.csv"))
...
# Add univariate regression results
for (i in 1:nrow(univariate_df)) {
  row <- univariate_df[i, ]
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H4-reverse",
      question = paste("Does", tolower(row$predictor), "predict coral condition?"),
      test_name = "Linear regression",
      test_statistic = row$f_statistic,
      df = paste("1,", row$n - 2),
      p_value = row$p_value,
      effect_size = row$r_squared,
      effect_type = "R²",
      n = row$n,
      notes = paste("β =", round(row$estimate, 3))
    )
  )
}

# Add CAFI PC1 result if available
if (exists("model_pca_condition")) {
  pca_sum <- summary(model_pca_condition)
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H4-reverse",
      question = "Does CAFI community composition (PC1) predict condition?",
      test_name = "Linear regression",
      test_statistic = pca_sum$fstatistic[1],
      df = paste(pca_sum$fstatistic[2], pca_sum$fstatistic[3], sep = ", "),
      p_value = pf(pca_sum$fstatistic[1], pca_sum$fstatistic[2], pca_sum$fstatistic[3], lower.tail = FALSE),
      effect_size = pca_sum$r.squared,
      effect_type = "R²",
      n = nrow(analysis_data),
      notes = "Community composition axis"
    )
  )
}

# Save standardized results
save_stats_summary(stats_results, "08_cafi_condition_feedbacks", "CAFI-Condition Feedback Analysis")
