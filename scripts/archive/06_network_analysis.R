#!/usr/bin/env Rscript
# ============================================================================
# 06_network_analysis.R - Co-occurrence Network Analysis
# ============================================================================
#
# PURPOSE: Build and analyze CAFI species co-occurrence networks
#
...
#   habitat preferences, and identifiable keystone species.
#
# ANALYSES:
...
#   - Modules reflect ecological similarity (e.g., Trapezia cluster)
#   - Keystone species identifiable through centrality metrics
#
# MANUSCRIPT FIGURES:
#   >>> FIGURE 4: Composition & Networks <<<
#   - Panel C: Co-occurrence network visualization
#   - Panel D: Network modularity statistics
#
...
fig_dir <- file.path(FIGURES_DIR, "networks")
...
if(length(cor_edges) > 0) {
  ...
  write_csv(edge_list,
            file.path(TABLES_DIR, "cafi_cooccurrence_pairs.csv"))
...
if (exists("edge_list") && nrow(edge_list) > 0) {
  ...
  V(g)$degree <- igraph::degree(g)                    # Number of connections
  V(g)$betweenness <- igraph::betweenness(g)         # Bridging position
  V(g)$closeness <- igraph::closeness(g)             # Average distance to all nodes
  V(g)$eigenvector <- igraph::eigen_centrality(g)$vector  # Influence based on connections

  # Network-level metrics
  network_metrics <- data.frame(
    n_cafi_species = igraph::vcount(g),
    n_associations = igraph::ecount(g),
    density = igraph::edge_density(g),
    transitivity = igraph::transitivity(g),
    mean_path_length = igraph::mean_distance(g),
  ...
  if(igraph::vcount(g) > 3) {
    ...
    network_metrics$modularity <- igraph::modularity(communities_louvain)
    network_metrics$n_modules <- length(unique(igraph::membership(communities_louvain)))
  }

  write_csv(network_metrics,
            file.path(TABLES_DIR, "cafi_network_metrics.csv"))
  ...
  if (igraph::vcount(g) > 10) {
    ...
    V(g)$module <- igraph::membership(communities)

    cat(sprintf("  ✓ Identified %d modules (modularity = %.3f)\n",
                max(V(g)$module), igraph::modularity(communities)))
    ...
    write_csv(module_summary,
              file.path(TABLES_DIR, "cafi_network_modules.csv"))
  ...
  keystone_scores <- data.frame(
    species = V(g)$name,
    type = V(g)$type,
    module = if("module" %in% vertex_attr_names(g)) V(g)$module else NA,
    degree = V(g)$degree,
  ...
  write_csv(keystone_scores,
            file.path(TABLES_DIR, "cafi_keystone_species.csv"))
...
if(exists("g") && igraph::vcount(g) > 3 && nrow(condition_scores) > 10) {
  ...
  coral_network_metrics <- coral_network_metrics %>%
    ...
    mutate(
      ...
      n_modules = if(length(cafi_in_network) > 0 && "module" %in% vertex_attr_names(g)) {
        n_distinct(V(g)$module[V(g)$name %in% cafi_in_network])
      } else NA

    ) %>%
    ungroup() %>%
    dplyr::select(-cafi_species, -cafi_in_network)  # Remove list columns

  # Merge with condition scores AND CORAL SIZE (use 'volume' from coral_master)
  coral_condition_network <- coral_network_metrics %>%
    inner_join(condition_scores %>% dplyr::select(coral_id, condition_score, site),
              by = "coral_id") %>%
    left_join(coral_master %>% dplyr::select(coral_id, volume),
              by = "coral_id") %>%
    rename(volume_lab = volume)

  # Save merged dataset
  write_csv(coral_condition_network,
            file.path(TABLES_DIR, "coral_condition_network_metrics.csv"))
  ...
  if(sum(!is.na(coral_condition_network$mean_degree)) > 20) {
    ...
    p_degree_condition <- coral_condition_network %>%
      ...
      geom_point(aes(color = site, shape = site), alpha = 0.7, size = 2.5) +
  ...
  if(sum(!is.na(coral_condition_network$has_keystone) &
    ...
    p_size_keystone <- coral_condition_network %>%
      ...
      geom_point(aes(color = site, shape = site), alpha = 0.5, size = 2.5,
                position = position_jitter(height = 0.05, width = 0)) +
      geom_smooth(method = "glm", method.args = list(family = "binomial"),
                 color = "black", se = TRUE, linewidth = 0.8) +
      scale_color_site() +
      scale_shape_site() +
  ...
  write_csv(size_site_results,
            file.path(TABLES_DIR, "coral_size_site_network_effects.csv"))
...
if(exists("g") && igraph::vcount(g) > 3 && nrow(coral_master) > 30) {
  ...
  for(site_name in sites) {
    ...
    if(nrow(site_comm) >= 10 && ncol(site_comm) >= 10) {
      ...
      if(length(site_edges) > 0) {
        ...
        # Create site network
        g_site <- igraph::graph_from_data_frame(site_edge_list[, c("sp1", "sp2", "strength")],
                                        directed = FALSE)
        ...
        site_networks[[site_name]] <- data.frame(
          ...
          n_cafi_species = igraph::vcount(g_site),
          n_edges = igraph::ecount(g_site),
          density = igraph::edge_density(g_site),
          transitivity = igraph::transitivity(g_site),
          modularity = if(igraph::vcount(g_site) > 3) {
            igraph::modularity(igraph::cluster_louvain(g_site))
          } else NA
        )
      }
    }
  ...
  if(length(site_networks) > 0) {
    ...
    write_csv(site_comparison,
              file.path(TABLES_DIR, "cafi_network_by_site.csv"))

    cat("  ✓ Site-specific networks characterized\n")

    # Visualize comparison
    p_site_compare <- site_comparison %>%
      dplyr::select(site, density, transitivity, modularity) %>%
      ...
      scale_fill_site() +
      labs(
        title = "CAFI network properties by site",
...
if(exists("g") && igraph::vcount(g) > 10) {
  ...
  for(i in 1:n_randomizations) {
    ...
    g_random <- igraph::erdos.renyi.game(igraph::vcount(g), igraph::edge_density(g), type = "gnp")

    random_metrics[i, "density"] <- igraph::edge_density(g_random)
    random_metrics[i, "transitivity"] <- igraph::transitivity(g_random)
    random_metrics[i, "mean_path_length"] <- igraph::mean_distance(g_random)
    random_metrics[i, "modularity"] <- igraph::modularity(igraph::cluster_louvain(g_random))
  }
  ...
  write_csv(null_comparison,
            file.path(TABLES_DIR, "network_null_model_comparison.csv"))
  ...
  if(exists("edge_list")) {
    ...
    write_csv(association_summary,
              file.path(TABLES_DIR, "association_type_summary.csv"))
  ...
  if(exists("module_summary")) {
    ...
    write_csv(module_composition,
              file.path(TABLES_DIR, "module_taxonomic_composition.csv"))
  ...
  write_csv(centralization_metrics,
            file.path(TABLES_DIR, "network_centralization.csv"))
  ...
  degree_assortativity <- igraph::assortativity_degree(g)

  # Type assortativity: do crabs co-occur with crabs, shrimp with shrimp, etc.?
  type_assortativity <- igraph::assortativity_nominal(g, as.factor(V(g)$type))
  ...
  write_csv(assortativity_results,
            file.path(TABLES_DIR, "network_assortativity.csv"))
...
if(exists("network_metrics") && !is.null(network_metrics)) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5_network",
      question = "What is the structure of the CAFI co-occurrence network?",
      test_name = "Network metrics",
      test_statistic = network_metrics$modularity,
      df = paste(network_metrics$n_cafi_species, "nodes,", network_metrics$n_associations, "edges"),
      p_value = NA,
      effect_size = network_metrics$density,
      effect_type = "Density",
      n = network_metrics$n_cafi_species,
      notes = sprintf("Modularity=%.3f, Transitivity=%.3f",
                      network_metrics$modularity, network_metrics$transitivity)
    )
  )
}

# 2. Null model comparison (modularity)
if(exists("null_comparison") && nrow(null_comparison) > 0) {
  mod_row <- null_comparison %>% filter(metric == "Modularity")
  if(nrow(mod_row) > 0) {
    stats_results <- bind_rows(stats_results,
      create_result_row(
        hypothesis = "H5a",
        question = "Is network modularity greater than random expectation?",
        test_name = "Z-test vs null model",
        test_statistic = mod_row$z_score,
        df = "1000 permutations",
        p_value = mod_row$p_value,
        effect_size = mod_row$observed - mod_row$random_mean,
        effect_type = "Diff from null",
        n = 1000,
        notes = sprintf("Obs=%.3f, Null=%.3f±%.3f",
                        mod_row$observed, mod_row$random_mean, mod_row$random_sd)
      )
    )
  }
}

# 3. Transitivity vs null
if(exists("null_comparison") && nrow(null_comparison) > 0) {
  trans_row <- null_comparison %>% filter(metric == "Transitivity")
  if(nrow(trans_row) > 0) {
    stats_results <- bind_rows(stats_results,
      create_result_row(
        hypothesis = "H5b",
        question = "Is network transitivity greater than random expectation?",
        test_name = "Z-test vs null model",
        test_statistic = trans_row$z_score,
        df = "1000 permutations",
        p_value = trans_row$p_value,
        effect_size = trans_row$observed - trans_row$random_mean,
        effect_type = "Diff from null",
        n = 1000,
        notes = sprintf("Obs=%.3f, Null=%.3f±%.3f",
                        trans_row$observed, trans_row$random_mean, trans_row$random_sd)
      )
    )
  }
}

# 4. Binomial test for positive associations
if(exists("binom_test")) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5c",
      question = "Are positive associations more common than expected by chance?",
      test_name = "Binomial test",
      test_statistic = binom_test$statistic,
      df = binom_test$parameter,
      p_value = binom_test$p.value,
      effect_size = binom_test$estimate,
      effect_type = "Proportion positive",
      n = nrow(edge_list),
      notes = sprintf("%.1f%% positive vs 50%% expected", binom_test$estimate * 100)
    )
  )
}

# 5. Chi-square for taxonomic clustering in modules
if(exists("chi_test")) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5d",
      question = "Is taxonomic composition non-random across network modules?",
      test_name = "Chi-square test",
      test_statistic = chi_test$statistic,
      df = chi_test$parameter,
      p_value = chi_test$p.value,
      effect_size = sqrt(chi_test$statistic / sum(contingency_table)),  # Cramér's V approximation
      effect_type = "Cramér's V",
      n = sum(contingency_table),
      notes = "Tests for taxonomic clustering within modules"
    )
  )
}

# 6. Condition vs mean degree
if(exists("coef_degree")) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5e",
      question = "Does hosting well-connected CAFI predict coral condition?",
      test_name = "Linear regression (condition ~ mean_degree + site)",
      test_statistic = coef_degree$estimate / coef_degree$std.error,
      df = NA,
      p_value = coef_degree$p.value,
      effect_size = coef_degree$estimate,
      effect_type = "β",
      n = NA,
      notes = "Mean degree of hosted CAFI species"
    )
  )
}

# 7. Size effect on network metrics
if(exists("coef_size") && !is.null(coef_size)) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5f",
      question = "Does coral size affect CAFI network connectivity?",
      test_name = "Linear regression (mean_degree ~ log(volume) + site)",
      test_statistic = coef_size$estimate / coef_size$std.error,
      df = NA,
      p_value = coef_size$p.value,
      effect_size = coef_size$estimate,
      effect_type = "β",
      n = NA,
      notes = "Effect of coral size on mean CAFI degree"
    )
  )
}

# 8. Assortativity metrics
if(exists("assortativity_results")) {
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5g",
      question = "Does the network show degree assortativity?",
      test_name = "Network assortativity coefficient",
      test_statistic = degree_assortativity,
      df = NA,
      p_value = NA,
      effect_size = degree_assortativity,
      effect_type = "Assortativity r",
      n = igraph::vcount(g),
      notes = assortativity_results$interpretation[1]
    )
  )
  ...
  stats_results <- bind_rows(stats_results,
    create_result_row(
      hypothesis = "H5h",
      question = "Does the network show taxonomic assortativity?",
      test_name = "Network assortativity coefficient (nominal)",
      test_statistic = type_assortativity,
      df = NA,
      p_value = NA,
      effect_size = type_assortativity,
      effect_type = "Assortativity r",
      n = igraph::vcount(g),
      notes = assortativity_results$interpretation[2]
    )
  )
}

# Save statistical results
save_stats_summary(stats_results, "06_network_analysis", "Network Analysis")
